# loads=c("shiny","maps","mapdata","rworldmap","geosphere","sp","lattice",
#         "XML","RColorBrewer","stringr","tm","leafletR","leaflet",
#         "scales","dplyr","rCharts","rJava")
# lapply(loads,require,character.only=T)
library(rCharts)
library(XML)
...
getNames=function(...){
  library(rworldmap)
  sPDF=getMap()
  auxnames=sPDF$NAME_SORT
  auxnames_=as.character(auxnames)
  for (i in grep(",",auxnames_)){
    auxnames_[i]=paste(strsplit(auxnames_[i],", ")[[1]][2],strsplit(auxnames_[i],", ")[[1]][1])
  }  
  auxnames_=gsub("S. ","South ",auxnames_)
  auxnames_=gsub("N. ","North ",auxnames_)
  auxnames_=data.frame(auxnames_)
  auxnames_$V2=rownames(auxnames_)
  colnames(auxnames_)=c("name","index")
  count=read.csv2("country.csv")
  auxnames__=NULL
  for (i in 1:nrow(auxnames_)){
    a=which(is.element(count$Country,auxnames_$name[i]))
    if (!length(a)==0){
      aux_=NULL
      if (length(strsplit(as.character(count$Adjective.of.Nationality[a]),"or")[[1]])>1){
        a_=strsplit(as.character(count$Adjective.of.Nationality[a])," or ")
        aux_$name=cbind(as.character(a_[[1]]))
      } else {
        aux_$name=as.character(count$Adjective.of.Nationality[a])
      }
      aux_$name=rbind(aux_$name,cbind(paste(aux_$name,"s",sep="")),paste(auxnames_$name[i],"'s",sep=""))
      aux_$index=i
      auxnames__=rbind(auxnames__,data.frame(aux_))
    }
  }
  out=rbind(auxnames_,na.omit(auxnames__))
